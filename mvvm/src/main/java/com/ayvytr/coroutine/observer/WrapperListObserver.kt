 ...
 */
abstract class WrapperListObserver<T>(private val iInit: IInit? = null) :
    Observer<ResponseWrapper<T>> {

    override fun onChanged(wrapper: ResponseWrapper<T>?) {
        if (wrapper!!.isSucceed) {
            onSucceed(wrapper.dataNonNull, wrapper.page, wrapper.isLoadMore, wrapper.hasMore)
        } else {
            onError(wrapper.exception!!)
        }
    }

    abstract fun onSucceed(data: T, page: Int, loadMore: Boolean, hasMore: Boolean)
